...
namespace DoubleMMPrjc
{
    ...
    public abstract class Entity : MonoBehaviour, TimerManager.IOnCountdownEnd
    {
        ...
        {
            Throw( attack.PushPower, attack.Direction, attack.PushDisableTime, blocked );
        }
        ...
        public virtual bool ShouldBlockAttack(Attack attack)
        {
            if (attack.Direction == Vector3.zero)
                return false;
            return Vector3.Angle( attack.Direction, LookRotation ) == 180;
        }
        ...
        public virtual void OnFallen(float speedWhenFalling)
        {
            if (isFallDamagable && speedWhenFalling > MIN_DAMAGEABLE_FALL_SPEED) {
                TakeDamage(
                    new Attack( ( speedWhenFalling - MIN_DAMAGEABLE_FALL_SPEED ) / MIN_DAMAGEABLE_FALL_SPEED,
                    true ) );
            }
        }
